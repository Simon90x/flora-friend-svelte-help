...
<aside
  class="flex flex-col h-screen sticky top-0 bg-white dark:bg-gray-700 text-gray-500 dark:text-gray-200 shadow-2xl w-70"
>
  <!-- Header -->
  <div class="p-4 pt-6 mb-4 flex-shrink-0">
    <a href="/dashboard" use:link on:click={onNavigate} class="flex items-center space-x-3 px-2">
      ...
      <span class="relative text-4xl font-semibold text-gray-800 dark:text-gray-100">FloraFriend</span>
    </a>
    <button
      on:click={() => isSidebarOpen.set(false)}
      class="lg:hidden absolute top-5 right-0 text-gray-500 hover:text-gray-800 dark:hover:text-gray-200"
  ...
  <nav class="flex-grow px-2 overflow-y-auto">
    <!-- Ripristinata la tua struttura esatta -->
    <ul class="space-y-6">
      <li>
        <a
          href="/dashboard"
          use:link
          on:click={onNavigate}
          class="flex items-center space-x-3 px-2 transition-colors text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-50"
        ...
        <a
          ...
          on:click={onNavigate}
          class="flex items-center space-x-3 px-2 transition-colors text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-50"
          class:active={$location.startsWith('/profile')}
    ...
    {#if $user}
      <div class="border-t border-gray-200 dark:border-gray-700 p-2">
        ...
        >
          <img
            src={$user.user_metadata?.avatar_url ||
              `https://ui-avatars.com/api/?name=${encodeURIComponent(($user.user_metadata && ($user.user_metadata.name || $user.user_metadata.full_name)) || $user.email)}&background=random`}
            alt="Avatar utente"
            class="w-10 h-10 rounded-full object-cover flex-shrink-0"
          />
          <div class="ml-3 flex-1 overflow-hidden">
            <p class="font-semibold text-sm truncate text-gray-800 dark:text-gray-100">
              {($user.user_metadata && ($user.user_metadata.full_name || $user.user_metadata.name)) || 'Utente'}
            </p>
        ...
        >
          ...
          <span>Logout</span>
<style>
  /* Rimuovi @apply: usa direttamente le classi Tailwind negli elementi */
</style>ly text-gray-900 dark:text-gray-50;
  }
