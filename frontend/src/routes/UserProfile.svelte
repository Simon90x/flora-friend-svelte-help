<script>
    ...
    async function handleSaveAvatar() {
        ...
        alert('Avatar salvato!'); // Sostituire con un toast in futuro
    }
...
<div class="max-w-4xl mx-auto space-y-8">
    ...
    {#if isDemo}
    <div class="bg-blue-100 dark:bg-blue-900/50 border-l-4 border-blue-500 text-blue-700 dark:text-blue-200 p-4 rounded-lg" role="alert">
        <p class="font-bold">Modalità Demo</p>
        <p>Stai visualizzando un profilo dimostrativo. Le modifiche non verranno salvate in un database, ma l'interfaccia reagirà come se lo facessero. Il "Cambio Password" è disabilitato.</p>
    </div>
    {/if}
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <div class="flex items-center space-x-4">
                    <img src={newAvatarBase64 || `https://ui-avatars.com/api/?name=${encodeURIComponent(currentUser.user_metadata?.name || currentUser.email)}&background=random`} alt="Avatar utente" class="w-20 h-20 rounded-full object-cover bg-gray-200">
    ...
    <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-6 rounded-lg">
        ...
        <p class="text-red-700 dark:text-red-300 mb-4">Questa azione è irreversibile e cancellerà il tuo account demo.</p>
        <Button variant="ghost" on:click={() => isConfirmDeleteOpen = true} className="bg-red-600 text-white hover:bg-red-700 focus:ring-red-500">
            Elimina Account
        </Button>
    </div>
</div>
